        ...
        <div class="card-header bg-white">
          <div class="row">
            <div class="col-md-10 font-weight-bold   form-inilne small">
                <h3>อนุมัติซ่อมบำรุง</h3>
            </div>
            <div class="col-md-2 text-right">
            {{-- @if (in_array( 'ma_approved.create', $Permissions)) --}}
              {{-- <a href="{{url('ma_approvedcreate')}}"  class="btn btn-facebook" id="create">สร้าง</a> --}}
            {{-- @endif --}}
            </div>
              ...
              <table class="table table-bordered table-hover table-sm table-striped small" id="dataTable">
                <thead class="text-center bg-primary text-white">
                  <tr>
                    ...
                    <th>ผู้รับผิดชอบการซ่อมบำรุง</th>  
                    <th>ฝ่าย</th>
                    <th>สร้างโดย</th>
                    <th>สถานะใบงาน</th>
...
console.log(startDate);
                ...
                try{
                    var return_data = new Array();         
                        var EditPermission = ' d-none '
                          ...
                          @if (in_array( 'ma_approved.edit', $Permissions))
                          EditPermission = 'd-inline';
                          @endif
                          @if (in_array( 'ma_approved.delete', $Permissions))
                          ...
                          var ap_ma_type = json.data[i]["ap_ma_type"] == "ma_in" ? 
                            '<span>ซ่อมภายใน</span>' : 
                          ...
                          return_data.push({ 
                              ...
                              'approved_by'   : json.data[i]["approved_by"],
                              'approved_dep'   : json.data[i]["approved_dep"],
                              'ap_ma_no'   : json.data[i]["ap_ma_no"],
                              'job_date'   : dateFormatddmmyyyy(json.data[i]["job_date"]),
                              'ma_date'   : dateFormatddmmyyyy(json.data[i]["ma_date"]),
                              'ap_ma_type'   : ap_ma_type,
                              'assign_as'   : json.data[i]["assign_as"],
                              'f_name'   : json.data[i]["f_name"] +'  '+ json.data[i]["l_name"],
                              'ap_request_by'   : json.data[i]["ap_request_by"],
                              'ap_request_dep'   : json.data[i]["ap_request_dep"],
                              'ap_request_tel'   : json.data[i]["ap_request_tel"],
                              'ap_asset_send'   : json.data[i]["ap_asset_send"],
                              'ap_asset_model'   : json.data[i]["ap_asset_model"],
                              'ap_asset_brand'   : json.data[i]["ap_asset_brand"],
                              'ap_asset_serial'   : json.data[i]["ap_asset_serial"],
                              'ap_asset_no'   : json.data[i]["ap_asset_no"],
                              'ap_asset_desc'   : json.data[i]["ap_asset_desc"],
    ...
    $('#dataTable tbody').on( 'click', '.btnedit', function () {
      ...
      window.location = 'ma_approved/' + _id + '/edit';
      console.log(data)
...
$('#dataTable tbody').on( 'click', '.btndelete', function () {
        ...
        document.getElementById("daleteFrm").action = "{{url('ma_approved')}}" + '/' + _id
});
